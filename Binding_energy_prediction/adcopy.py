...
from peti import modified_pettifor
from ase import io
os.chdir('adsorbed')
f=glob.glob('*cif*')
type_encoding = {}
...
for Z in tqdm(range(1, 119), bar_format=bar_format):
    ...
    print(specie.symbol,specie.mass)
    val=modified_pettifor(specie.symbol)
    specie_am.append(val)
 #   import sys
#    sys.exit(1)
...
index_train=list(arr[:734])
index_test=list(arr[734:834])
index_valid=list(arr[834:])
...
def doit(name1,nm,dataloader):
    ...
    # In[16]:
    name2=name1+nm+str(ep)+'_epochs.csv'

    df.to_csv(name2,index=False)
doit(run_name,"_train_",dataloader_train)
doit(run_name,"_test_",dataloader_test)
