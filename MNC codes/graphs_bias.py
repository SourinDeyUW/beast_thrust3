         ...
         #   print('wo',z)
            ...
            for sk in range(len(z)):
                if sk==0:
                    ...
                    ss.append([df['bias'][ks]])
           # print(ss)
            ss=torch.tensor(ss)
           # ga=torch.tensor([[df['jid'][ks]] for p1 in range(len(z))])
            g.ndata['bias']=ss#torch.tensor(df['bias'][ks])
           # print(g.ndata['bias'])
          #  import sys;sys.exit(1)
            z = z.type(torch.IntTensor).squeeze()
           # print(df['jid'][ks],'oo')
            #import sys;sys.exit(1)
           # g.ndata['jid']=df['jid'][ks]            
            f=torch.tensor(df['nf'][ks]).float()
           # print('tag_graph')
            ks+=1
