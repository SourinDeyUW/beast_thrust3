...
class ALIGNN(nn.Module):
    ...
    def __init__(self, config: ALIGNNConfig = ALIGNNConfig(name="alignn")):
        ...
        if self.classification:
            ...
            self.fc = nn.Linear(config.hidden_features+1, config.output_features)
        self.link = None
    ...
    def forward(
       ...
       # print(g.ndata['atom_features'].size())
        g = g.local_var()
 #       print(g.ndata)
       # print(g.ndata['atom_features'].size())
       # print('whoa')
       ...
       # h1=h[:,0].reshape(16,1)
      #  print(h1)
       # print(g.ndata['bias'],len(g.ndata['bias']))
        qsl=list(g.ndata['bias'])
        qsl=[qq.item() for qq in qsl]
        #print('checked')
       # print(qsl[1])
        qs=[iq.detach().cpu() for iq in g.ndata['bias'] if iq>1]
        qs2=[qsl[iq+1] for iq in range(len(qsl)) if qsl[iq]>1 ]
        h1=h[:,0].clone().reshape(len(qs2),1)
        qs2=torch.tensor(qs2).reshape(len(qs2),1)
        for pt in range(len(qs2)):
            h1[pt]=qs2[pt]
        #h1=torch.nn.Linear(1,1,device='cuda:0')(h1)
       # print(h1,h1[0])
       # print('**'*10)
        #import sys;sys.exit(1)
        #for e1 in range(16):
        #   h[e1]=torch.cat((h[e1],h1),dim=1)
        h=torch.cat((h,h1),dim=1)
       # h=h2
       # print(h.size())
       # print(h1[0][254],h[0][254],'ok')
        #import sys;sys.exit(1)
        #f2=torch.tensor([1.2]).to('cuda:0')
        #print(h[0].size())
        #for k1 in range(16):
         #   h[k1]=torch.cat((h[k1],f2),dim=0)
        #import sys;sys.exit()
        out = self.fc(h)
